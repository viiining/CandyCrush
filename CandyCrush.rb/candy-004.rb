...
def humanReadableTimer(seconds)
  hour = (seconds / 3600).floor.to_s;
  minute = ((seconds % 3600) / 60).floor.to_s
  second = (seconds % 60).to_s;

  [hour, minute, second].map {|element| element.rjust(2, "0")}.join(":")
end
...
# solving steps:
# 將數字換成 hour, minute, second，以陣列做 map 與 rjust 方法，讓每個元素都設定兩位，不足則補零，最後再以 ":" join 為字串。